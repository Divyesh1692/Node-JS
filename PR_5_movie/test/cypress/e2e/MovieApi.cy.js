describe("Movie", () => {
  ...
  it("should update a movie - marks 1", () => {
    ...
    cy.request("PATCH", `/movie/update/${movieId}`, updateData).then(
      (response) => {
        expect(response.status).to.equal(200);
  ...
  it("should add a rating to a movie (0 to 10) - marks 1", () => {
    ...
    cy.request("PATCH", `/movie/rating/${movieId}`, ratingData).then(
      (response) => {
        expect(response.status).to.equal(200);
        // Add more assertions as needed
        expect(
          response.body.ratings[response.body.ratings.length - 1]
        ).to.have.property("value", 8);
      }
    );
  });
  ...
  it("should add a comment to a movie - marks 1", () => {
    ...
    cy.request("PATCH", `/movie/comment/${movieId}`, commentData).then(
      (response) => {
        expect(response.status).to.equal(200);
        // Add more assertions as needed
        expect(
          response.body.comments[response.body.comments.length - 1]
        ).to.have.property("text", "Great movie!");
      }
    );
  });
