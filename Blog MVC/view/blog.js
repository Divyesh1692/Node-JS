...
document
  ...
  .addEventListener("submit", handleData);
async function fetchBlogPosts() {
  const response = await fetch("http://localhost:2005/blogs/blog");
...
if (!userId) {
  window.location.href = "http://127.0.0.1:5500/view/blog.html";
}
...
async function deleteBlogPost(id) {
  const response = await fetch(`http://localhost:2005/blogs/blog/${id}`, {
    method: "DELETE",
  });
  fetchBlogPosts();
}
